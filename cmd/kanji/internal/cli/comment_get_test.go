...
func TestCommentGet_ByID(t *testing.T) {
	dir := t.TempDir()
	...
	comment, err := rt.CommentService.AddComment(context.Background(), application.AddCommentInput{
		TaskID:     task.ID,
		ProviderID: setup.Provider.ID,
		BodyMD:     "Test comment body",
	})
	require.NoError(t, err)
	rt.Close()
	...
	require.NoError(t, cmd.ParseFlags([]string{"--db-path", dbPath, "--comment-id", comment.ID}))
	buf := new(strings.Builder)
	cmd.SetOut(buf)

	ns := Namespace{Key: "test-ns", Source: "cwd"}
	err = runCommentGet(cmd, ns)
	require.NoError(t, err)

	output := buf.String()
	assert.Contains(t, output, comment.ID)
	assert.Contains(t, output, "Test comment body")
}
...
func TestCommentGet_NotFound(t *testing.T) {
	...
	cmd := &cobra.Command{}
	cmd.Flags().String("db-path", "", "")
	cmd.Flags().String("comment-id", "invalid", "")
	require.NoError(t, cmd.ParseFlags([]string{"--db-path", dbPath, "--comment-id", "invalid"}))
	buf := new(strings.Builder)
	cmd.SetOut(buf)
...
func TestCommentGet_JSON(t *testing.T) {
	dir := t.TempDir()
	dbPath := filepath.Join(dir, "test.db")
	...
	require.NoError(t, cmd.ParseFlags([]string{"--db-path", dbPath, "--comment-id", comment.ID, "--json"}))
	buf := new(strings.Builder)
	cmd.SetOut(buf)
